...
2. **The gold seal is mandatory and non-configurable.** Every certificate template must render the shared `<GoldSeal />` component (`lib/certificates/GoldSeal.tsx`) at a fixed position — there is no standalone SVG asset file; the component is the canonical implementation. Issuer brand settings must never be able to remove, recolor, or resize it.
