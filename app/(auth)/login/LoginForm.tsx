...
//
// `?next=` (docs/build-phases.md Phase 8) lets a caller — currently just
// /claim/[token] — send the user back to a specific page after signing in,
// instead of the default /apply landing spot. Defaults to /apply so every
// other existing link into this page (none of which pass `next`) behaves
// exactly as before.
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-6 p-8">
      ...
      <p className="text-sm text-certified-muted">
        No account?{' '}
        <Link href={`/signup${next !== '/apply' ? `?next=${encodeURIComponent(next)}` : ''}`} className="text-certified-navy underline">
          Sign up
        </Link>
      </p>
