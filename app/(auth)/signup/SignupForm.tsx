...
// /claim/[token] — send a brand-new account back to a specific page after
// email confirmation, instead of the default /apply. Defaults to /apply so
// every other existing link into this page behaves exactly as before.
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-6 p-8">
      ...
      <p className="text-sm text-certified-muted">
        ...
        <Link href={`/login${next !== '/apply' ? `?next=${encodeURIComponent(next)}` : ''}`} className="text-certified-navy underline">
          Sign in
        </Link>
      </p>
    </main>
