      ...
      <div className="flex flex-col gap-6">
        ...
        <ol className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          <Step n={1} title="Apply as a business or individual trainer">
            Submit your legal or registered name, an identification document, and, for businesses, proof of
            operation, such as a registration certificate. Individual trainers without a registered business instead
            sign a declaration of the training they deliver.
          </Step>
          ...
          <Step n={4} title="Issue certificates">
            Add trainees one at a time, or upload a CSV for a whole graduating cohort. Each one gets a certificate
            PDF, a unique verification record, and, with consent, a public directory profile, instantly.
          </Step>
        </ol>
      </div>
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          <Card>
            ...
            <p className="mt-2 text-sm text-certified-muted">
              Every certificate carries a QR code and a short public ID, e.g. <code>CERT-8F2K9-XQ41</code>, random,
              not sequential, so IDs can't be guessed or scraped in order.
            </p>
          </Card>
          <Card>
            <p className="font-semibold text-certified-navy">The record, not the PDF, is authoritative</p>
            <p className="mt-2 text-sm text-certified-muted">
              A cryptographic signature computed over the certificate's core fields at issuance is re-checked on
              every lookup, a direct database edit without going through the signing service would invalidate it.
            </p>
          </Card>
          <Card>
...
function Step({ n, title, children }: { n: number; title: string; children: React.ReactNode }) {
  return (
    <div className="flex gap-4 rounded-card border border-certified-border bg-certified-surface p-5 dark:bg-white/[0.04] dark:backdrop-blur-xl">
      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-certified-navy text-sm font-semibold text-white dark:bg-blue-600">
        {n}
      </span>
      <div>
