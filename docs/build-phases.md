# Certified Africa — Build Phases & Prompts
...
| 3 | **Supabase** | Create a new project (choose a region with good latency across Africa/Europe, e.g. `eu-west`). Save the project URL, anon key, and service role key from Settings → API. Enable Email + Google providers under Authentication → Providers. Enable TOTP under Authentication → MFA. Also install the Supabase CLI and run `supabase init` + `supabase start` for local development — this is your default dev target, not the hosted project. | Free tier: 500MB DB, 1GB storage, 50k MAU — plenty at launch, but local dev is still the default per `CLAUDE.md`. |
...
**Prompt:**
> Build the application flow: sign-up → choose "Business/Training Centre" or "Individual Trainer" → structured multi-step form capturing the fields in `docs/blueprint.md` §3.1: legal/registered name, optional RC/CAC number, identification document upload (any means of identification — business/work ID card, government-issued ID, etc. — required for both types), proof-of-operation (CAC certificate upload, required for Business/Training Centre, optional for Individual Trainer), business address (street/state/LGA), owner contact (full name, phone, email), expected trainee volume band (0-5 / 6-15 / 16-29 / 30+), field(s) of training, and a short public description. For "Individual Trainer," additionally render the full text from `docs/declaration-form.md` with a required checkbox, and store `declaration_signed_at`, `declaration_version`, and `declaration_submission_ip` alongside the identification document reference, as described in that doc's implementation note. Handle file uploads (identification document, proof-of-operation) to Supabase Storage in a private bucket, re-encoding any image uploads server-side (strip EXIF, re-save as clean JPEG/WebP) before storing — PDF uploads (e.g. a CAC certificate) stay in the private bucket as-is, no re-encoding needed since EXIF isn't a PDF concern. Add the Altcha widget to the final submit step, with server-side challenge verification (`altcha-lib`) on the submit handler. On submit, create an `Organization` (status: pending) and `Application` record. Show the applicant a status page reflecting their current state (pending / more info requested / approved / rejected) with any agent comments visible.
...
**Prompt:**
> Build the public directory at `/directory`: search/filter by field/skill, country + region + locality (structured Country dropdown across all of Africa, structured Region dropdown for priority countries — Nigeria, Ghana, Kenya, South Africa — free text elsewhere, per `docs/blueprint.md` §7 and `lib/geo/africa.ts`), issuer, and completion-date range. Use Postgres full-text search / indexed columns — no external search service. Build the trainee public profile page showing name, photo, bio, certificate(s), issuing org with "Approved Issuer" badge, and an "open to hire" toggle state. Build the issuer's own public page listing their programs and trainee roster. Ensure both are excellent on mobile (design system §3) since verification/discovery traffic is QR/phone-first.
