...
3. **Flutterwave: the credentials provided are v4, live, and architecturally incompatible with what was built.** `lib/payments/flutterwave.ts` targets v3 (a static secret key, `POST /v3/payments` returning a hosted checkout `link`). v4 uses OAuth2 client-credentials (verified live — a real token fetch against `https://idp.flutterwave.com/realms/.../token` with the provided `client_id`/`client_secret` succeeded) and, per `developer.flutterwave.com`'s own docs as of this date, **has no hosted-checkout/payment-link endpoint at all** — real charges require the merchant to build its own raw card-entry UI, encrypt card number/expiry/CVV client-side with AES-256-GCM using the provided encryption key, POST to `/orchestration/direct-charges`, and handle a `next_action` (3DS redirect, OTP, etc.). That's real new PCI-adjacent scope, not a key swap — and these are **live** credentials, so a mistake here processes a real charge, not a sandbox one. Deliberately did **not** build this against live keys without asking first: Flutterwave is disabled in the UI (`BuyCreditsForm.tsx`) and rejected server-side (`app/dashboard/billing/actions.ts`) with a clear "temporarily unavailable" message, rather than shipping a silently-broken or riskily-rushed option. Credentials are saved in `.env.local` under their real v4 names (`FLUTTERWAVE_CLIENT_ID`/`FLUTTERWAVE_CLIENT_SECRET`/`FLUTTERWAVE_ENCRYPTION_KEY`) for whichever way the decision below goes. **Paystack-only for now, both in the UI and enforced server-side.**
4. **Live-verified (safe, zero money moved):** Paystack's full adapter round-trip against the real API — `createCheckout` returned a real `checkout.paystack.com` URL, `verifyTransaction` on the un-paid reference correctly read back a non-success status. Flutterwave's OAuth token endpoint returned a valid access token for the live `client_id`/`client_secret`, confirming the credentials are active — no charge/orchestration endpoint was touched at all, on purpose, given they're live keys.
...
7. Not deployed — this is on a feature branch, not merged to `main` yet, let alone redeployed to Vercel. **Not merged yet either** — the user asked for a merge "after confirmation that everything runs clean and perfect," which is honestly true for Paystack but not yet for Flutterwave; flagged back rather than merging with a known-disabled provider silently glossed over.
