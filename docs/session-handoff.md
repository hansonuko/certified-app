...
(`public/brand/certified-seal.png`, `certified-logo.png`) across every
...
**1. Real seal/logo artwork, replacing hand-drawn code.** User-supplied images — `public/brand/certified-seal.png` (the gold medallion) and `certified-logo.png` (the Africa-map wordmark) — replace what were previously code-drawn SVG (the seal, `lib/certificates/GoldSeal.tsx` / `components/GoldSeal.tsx`) and styled text (the header/footer logo, `components/Logo.tsx`). CLAUDE.md rule #2 and `docs/design-system.md` §5 updated to describe the new implementation — the rule itself (mandatory, non-configurable, fixed position, never recolored/resized by issuer brand settings) is unchanged, only what backs it.
...
**Update, same session — background removed, real transparency.** The images as first supplied each sat on a light square/card backdrop (a "mockup" rendering style) — the first pass hid that with CSS (circle-crop + `object-fit: cover` for the seal, a white rounded chip behind the logo) rather than removing it, which the user then asked for directly, plus a small size bump on the logo ("increase it a little to be visible"). No AI background-removal service was available offline, so this used a hand-rolled two-stage pixel algorithm (`sharp`'s raw buffer access, no new npm dependency): (1) a contiguous flood-fill seeded from every border pixel, chaining a small per-step color-distance tolerance so it correctly follows the backdrop's gradient/vignette without needing one global threshold; (2) for the logo specifically, a morphological "opening" pass (erode then dilate the background mask) to sever thin connections between the backdrop and the logo's own white letter-fill color — without it, flood-fill punched transparent holes into "Certified"/"Africa"'s white-colored letter interiors, since parts of the lettering touch the backdrop through narrow gaps. The seal didn't need this step (a plain coin on a plain backdrop, no thin-channel problem) and looked worse with it (eroded fine detail on the textured rope-twist ring edge) — tuned per-image rather than one shared setting. A final small-debris cleanup pass discards only genuinely tiny disconnected specks (JPEG-compression shadow artifacts), not "keep only the single largest region" (tried first, wrongly deleted real disconnected design elements — individual letters, the decorative triangle arc — since neither image's subject is one 4-connected blob). Re-exported as palette-optimized PNG (`sharp`'s `palette: true`) — ended up *smaller* than the original JPEGs despite gaining an alpha channel (seal 173KB→195KB, logo 690KB→120KB). Both `GoldSeal` components dropped their circle-crop styling (no longer needed — the PNG's own transparency already delivers a clean edge) and `Logo.tsx` dropped the white chip wrapper entirely, confirmed visually clean against both a magenta and a dark test backdrop before wiring in. Logo bumped from `h-8`/`sm:h-9` to `h-11`/`sm:h-12`. `public/brand/*.jpg` replaced with `*.png`; CLAUDE.md/design-system.md references updated to match. Verified `tsc`/`build`/`test` clean and the header logo confirmed rendering correctly, larger and cleanly, in the browser — did **not** spend a second real certificate credit re-verifying the PDF seal for this refinement (same react-pdf `<Image>` pattern already proven working moments earlier, just simplified styling — `objectFit: contain` in place of the crop/border-radius combo — a strictly lower-risk change, not indistinguishable-from-untested).
